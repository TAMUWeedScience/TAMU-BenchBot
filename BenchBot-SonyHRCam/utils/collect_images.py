...
class CollectImages:
    ...
    def __init__(self, cfg):
        ...
        if Path(path2usb).exists():
            ...
            log.info(f"Final save location found. Creating directory at {self.final_batch_dir}")
    ...
    def file_rename(self, file_stem):
        ...
        for file_name in os.listdir("."):
            if file_name.startswith('DSC'):
                ...
                shutil.move(file_name, Path(self.final_batch_dir,new_name))
            
            """
            if Path(self.final_batch_dir,new_name).exists():
                log.info(f"Images were saved in usb.)
            else:
                log.info(f"Images were not saved in usb.")
